#!/usr/bin/env node
/**
 * EXTREME break-the-app live probes (abdull-ah-med + aqsa-05 ONLY).
...
console.log("=== EXTREME BREAK LIVE PROBES (abdull + aqsa) ===\n");
...
let abdTok = "";
let aqsaTok = "";
const tokens = () => [abdTok, aqsaTok].filter(Boolean);
...
try {
  console.log("0) Setup");
  {
    ...
    if (r.status !== 0) throw new Error("add aqsa: " + redact(r.stderr));
    acct(["bind", aqsaRoot, AQSA.id], env);
    acct(["install"], env);
    abdTok = ghToken(ABDULL.githubUser);
    aqsaTok = ghToken(AQSA.githubUser);
    ok("setup");
  }
  ...
  console.log("\nP1) Host/port/conflict parser (I7)");
  {
    ...
    for (const [name, body] of cases) {
      ...
      if (is443 || agreeDup) {
        if (leaked && out.includes(abdTok) && !out.includes(aqsaTok)) {
          ok(`${name} → abdull token only`);
        } else if (!leaked) {
  ...
  console.log("\nP2) Nearest .acct walk-up / nesting");
  {
    ...
    fs.writeFileSync(path.join(personalRoot, "a", ".acct"), "profile: aqsa\n");
    let r = acct(["status"], env, deep);
    if (/aqsa-05/.test(r.stdout) && /reason: local/.test(r.stdout)) ok("walk-up finds parent .acct");
    else fail("walk-up .acct", r.stdout);

    fs.writeFileSync(path.join(deep, ".acct"), "profile: personal\n");
    r = acct(["status"], env, deep);
    if (/abdull-ah-med/.test(r.stdout) && /reason: local/.test(r.stdout)) ok("nearest .acct wins over ancestor");
    ...
    // Sibling under a/ should still see aqsa from a/.acct
    const sib = path.join(personalRoot, "a", "sib");
    fs.mkdirSync(sib, { recursive: true });
    r = acct(["status"], env, sib);
    if (/aqsa-05/.test(r.stdout)) ok("sibling inherits ancestor .acct");
  ...
  console.log("\nP3) GIT_DIR / worktree confusion");
  {
    const repo = path.join(aqsaRoot, "wt-main");
    fs.mkdirSync(repo, { recursive: true });
    run("git", ["init"], { cwd: repo, env });
    run("git", ["config", "user.email", AQSA.email], { cwd: repo, env });
    run("git", ["config", "user.name", AQSA.name], { cwd: repo, env });
    fs.writeFileSync(path.join(repo, "f.txt"), "1\n");
    run("git", ["add", "f.txt"], { cwd: repo, env });
    run("git", ["commit", "-m", "init"], { cwd: repo, env });

    // Point GIT_DIR at aqsa repo while cwd is personal
    ...
    } else if (r.stdout.includes(aqsaTok)) {
      note(
        "error",
        "LOOPHOLE: GIT_DIR redirected helper to aqsa while cwd is personal",
        redact(r.stdout).slice(0, 150),
      );
      fail("GIT_DIR confusion", "aqsa token in personal cwd");
    } else fail("GIT_DIR unexpected", redact(r.stdout));

    // .acct in personal + GIT_DIR in aqsa
    ...
    if (r2.stdout.includes(abdTok) && !r2.stdout.includes(aqsaTok)) ok(".acct+cwd beats GIT_DIR");
    else if (r2.stdout.includes(aqsaTok)) {
  ...
  console.log("\nP4) Exec deny-list creative bypasses (I18)");
  {
    const attempts = [
      ...
      ["xargs", "-n2", "gh"],
      ["env", "xargs", "-n2", "gh"],
      ["xargs", "-I{}", "sh", "-c", "unset GH_TOKEN GITHUB_TOKEN; gh auth {} --user aqsa-05"],
      ["nice", "gh", "auth", "token"],
      ["nohup", "gh", "auth", "token"],
      ["time", "gh", "auth", "token"],
      ...
      ["gh", "auth", "login"],
      ["gh", "auth", "refresh"],
      ["gh", "auth", "switch", "--user", AQSA.githubUser],
      ["gh", "auth", "setup-git"],
      ["gh", "auth", "logout"],
      ...
      ["bash", "-c", "unset GH_TOKEN GITHUB_TOKEN; x=auth; gh $x switch --user aqsa-05"],
  ...
  console.log("\nP5) shell-env injection / profile id metacharacters");
  {
    ...
    if (/unset GH_TOKEN/.test(r.stdout) && !r.stdout.includes(abdTok) && !r.stdout.includes(aqsaTok)) {
  ...
  console.log("\nP6) Burst helper isolation");
  {
    let bad = 0;
    for (let i = 0; i < 40; i++) {
      const cwd = i % 2 === 0 ? personalRoot : aqsaRoot;
      const expect = i % 2 === 0 ? abdTok : aqsaTok;
      const other = i % 2 === 0 ? aqsaTok : abdTok;
      const r = helper("get", "protocol=https\nhost=github.com\n\n", cwd, env);
  ...
  console.log("\nP7) secrets.json hygiene / symlink swap");
  {
    ...
    if (!fs.existsSync(secrets)) fail("secrets missing", "no file");
      ...
      run(process.execPath, [ACCT, "profile", "token", ABDULL.id, "--stdin"], {
        env,
        input: abdTok + "\n",
      });
      run(process.execPath, [ACCT, "profile", "token", AQSA.id, "--stdin"], {
        env,
        input: aqsaTok + "\n",
      });
    }
  ...
  console.log("\nP8) Bind path tricks");
  {
    ...
    let r = acct(["bind", rel, ABDULL.id], env);
    if (r.status === 0) {
      const st = acct(["status"], env, rel);
      if (/abdull-ah-med/.test(st.stdout)) ok("absolute bind works");
      else fail("bind status", st.stdout);
    } else fail("bind", r.stderr);

    // Bind overlapping paths: parent personal, child should be able to rebind aqsa
    const child = path.join(personalRoot, "child-rebind");
    fs.mkdirSync(child, { recursive: true });
    r = acct(["bind", child, AQSA.id], env);
    const st = acct(["status"], env, child);
    if (/aqsa-05/.test(st.stdout)) ok("child rebind to aqsa (longest wins)");
    else fail("child rebind", st.stdout);

    // Path with .. that escapes
    const escape = path.join(personalRoot, "x", "..", "..", "aqsa");
    const st2 = acct(["status"], env, path.normalize(escape));
    if (/aqsa-05/.test(st2.stdout)) ok("normalized .. path resolves to aqsa tree");
    else {
  ...
  console.log("\nP9) Live cross-account API ACL");
  {
    ...
    if (r.status === 0) ok("abdull API list works");
    else fail("abdull API", redact(r.stderr));

    // Can aqsa use abdull token via wrong helper? already covered — try gh with swapped
    const wrong = run("gh", ["api", "user", "--jq", ".login"], {
      env: { ...env, GH_TOKEN: abdTok, GITHUB_TOKEN: undefined },
      cwd: aqsaRoot,
    });
    if (wrong.stdout.trim() === ABDULL.githubUser) {
      note(
        "info",
    ...
    const viaExec = acct(["exec", "gh", "api", "user", "--jq", ".login"], env, aqsaRoot);
    if (viaExec.stdout.trim() === AQSA.githubUser) ok("acct exec forces aqsa in aqsa tree");
    else fail("acct exec aqsa", viaExec.stdout);
  }
  ...
  console.log("\nP10) Debug + doctor hygiene");
  {
    for (const cmd of [["status"], ["whoami"], ["doctor"], ["profile", "list"], ["shell-env"]]) {
      ...
      if (all.includes(abdTok) || all.includes(aqsaTok)) fail(`${cmd.join(" ")}+debug leaked`, "token");
  ...
  {
    const repo = path.join(aqsaRoot, "hook-extreme");
    fs.mkdirSync(repo, { recursive: true });
    run("git", ["init"], { cwd: repo, env });
    run("git", ["config", "user.email", AQSA.email], { cwd: repo, env });
    run("git", ["config", "user.name", AQSA.name], { cwd: repo, env });
    ...
    if (r.status !== 0) ok("pre-commit blocks abdull email in aqsa tree");
    else fail("pre-commit email", "allowed wrong");

    run("git", ["config", "user.email", AQSA.email], { cwd: repo, env });
    r = acct(["hook-run", "pre-commit"], env, repo);
    if (r.status === 0) ok("pre-commit allows aqsa email");
    else fail("pre-commit allow", r.stderr);

    // Ambient ACCT_PROFILE=personal must not flip pre-push expected principal
    r = acct(["hook-run", "pre-push"], { ...env, ACCT_PROFILE: ABDULL.id }, repo);
    const out = r.stdout + r.stderr;
    if (/requires .*contactabdullahahmed|expected.*abdull/i.test(out) && r.status !== 0) {
  ...
  {
    const dest = path.join(aqsaRoot, "acct-public-clone");
    const r = run(
      "git",
      [
        "-c",
        ...
        "ls-remote",
        `https://github.com/${ABDULL.githubUser}/acct.git`,
        "HEAD",
      ],
      { cwd: aqsaRoot, env },
    );
    if (r.status === 0) {
      ok("aqsa creds can ls-remote public abdull/acct (expected)");
      note(
        "info",
