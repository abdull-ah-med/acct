 ...
 * Allowed: abdull-ah-med, aqsa-05 ONLY. Never touch Mair.
...
console.log("=== ADVERSARIAL LIVE PROBES (abdull + aqsa) ===\n");
...
for (const d of [personalRoot, aqsaRoot, unboundRoot, sibling]) fs.mkdirSync(d, { recursive: true });
fs.writeFileSync(
  gitconfig,
...
try {
  ...
  if (r.status !== 0) throw new Error("add aqsa failed: " + redact(r.stderr));
  ...
  console.log("\nA) .acct without git repo under bound tree");
  {
    ...
    fs.writeFileSync(path.join(dir, ".acct"), "profile: aqsa\n");
  ...
  {
    const realAqsa = path.join(base, "real-aqsa-tree");
    fs.mkdirSync(realAqsa, { recursive: true });
    acct(["bind", realAqsa, AQSA.id], env);
    const link = path.join(personalRoot, "via-symlink");
    try {
      fs.symlinkSync(realAqsa, link);
      r = acct(["status"], env, link);
      // Depending on normalizePath: may resolve to aqsa (good) or personal (bad)
      if (/aqsa-05/.test(r.stdout)) ok("symlink into aqsa tree resolves aqsa");
      else {
        note(
          "warn",
  ...
  console.log("\nC) Credential protocol smuggling");
  {
    ...
    for (const c of cases) {
      ...
      const out = r.stdout;
      const leaked = out.includes(abdTok) || out.includes(aqsaTok);
      ...
      if (c.name === "url= with userinfo") {
        if (leaked && out.includes(abdTok) && !out.includes(aqsaTok)) {
          ok("url= userinfo → host github.com → abdull token only (protocol OK)");
        } else if (!leaked) {
      ...
      if (leaked && !/host=github.com\n|host=GitHub.COM/i.test(c.body) && c.name !== "uppercase host") {
        ...
        if (out.includes(`password=${abdTok}`)) {
          ok(`uppercase host accepted (case-insensitive) — token only abdull`);
        } else fail(`uppercase host odd`, redact(out));
      } else if (leaked && c.name === "capability + get confusion") {
        if (out.includes(abdTok) && !out.includes(aqsaTok)) ok("capability attrs still return correct token");
  ...
  console.log("\nD) ACCT_DEBUG must not print real tokens");
  {
    ...
    if (r.stderr.includes(abdTok) || r.stderr.includes(aqsaTok)) {
    ...
    } else if (!r.stderr.includes(abdTok) && !r.stderr.includes(aqsaTok)) {
  ...
  console.log("\nE) Token exfil / env leakage");
  {
    ...
    if (r.stdout === abdTok && r.stdout !== aqsaTok) ok("exec overrides attacker GH_TOKEN");
    ...
    const bypasses = [
      ...
      ["exec", "sh", "-c", "gh auth token --user abdull-ah-med"],
      ["exec", "bash", "-c", '"gh" "auth" "token"'],
      ["exec", "bash", "-c", "g=gh; $g auth token"],
      ["exec", "bash", "-c", "x=auth; gh $x switch --user aqsa-05"],
      ["exec", "bash", "-c", "unset GH_TOKEN GITHUB_TOKEN; x=auth; gh $x switch --user aqsa-05"],
    ...
    for (const args of bypasses) {
      ...
      const dumped = out.includes(abdTok) || out.includes(aqsaTok);
  ...
  console.log("\nF) Hook bypass surfaces (I15 advisory)");
  {
    const repo = path.join(aqsaRoot, "bypass-repo");
  ...
  {
    // Clone abdull public repo into aqsa tree — helper should still yield aqsa token
    r = helper(
      "get",
      `protocol=https\nhost=github.com\npath=${ABDULL.githubUser}/acct.git\n\n`,
      aqsaRoot,
      env,
    );
    if (r.stdout.includes(aqsaTok) && !r.stdout.includes(abdTok)) {
      ok("cloning abdull path from aqsa tree still uses aqsa creds (cwd wins)");
      note(
        "info",
  ...
  console.log("\nH) Binding path prefix attacks");
  {
    ...
    if (/unbound|profile: \(unbound\)/.test(r.stdout) || !/abdull-ah-med/.test(r.stdout)) {
  ...
  console.log("\nI) erase / get isolation");
  {
    ...
    if (g.stdout.includes(abdTok)) ok("erase with foreign token did not clear abdull");
  ...
  console.log("\nJ) CLI output token hygiene");
  {
    for (const cmd of [["status"], ["whoami"], ["doctor"], ["profile", "list"]]) {
      ...
      if (all.includes(abdTok) || all.includes(aqsaTok)) {
  ...
  {
    r = acct(["profile", "token", ABDULL.id, "--stdin"], { ...env }, personalRoot);
    // empty stdin
    r = run(process.execPath, [ACCT, "profile", "token", ABDULL.id, "--stdin"], {
      env,
      cwd: personalRoot,
      input: "",
    });
  ...
  // ---------- M: Live API: can aqsa read abdull private e2e repo if leftover? ----------
  console.log("\nM) Cross-account API ACL (live)");
  {
    for (const name of ["acct-e2e-msl0623l-abd", "acct-e2e-msl0623l-aqsa"]) {
      const abdSee = run("gh", ["api", `repos/${ABDULL.githubUser}/${name}`, "--jq", ".full_name"], {
        env: { ...env, GH_TOKEN: abdTok, GITHUB_TOKEN: undefined },
      });
      const aqsaSeeAbd = run("gh", ["api", `repos/${ABDULL.githubUser}/${name}`, "--jq", ".full_name"], {
        env: { ...env, GH_TOKEN: aqsaTok, GITHUB_TOKEN: undefined },
      });
      if (name.includes("-abd")) {
        if (abdSee.status === 0) {
          note("warn", `Leftover throwaway still exists: ${ABDULL.githubUser}/${name}`, "delete manually — token lacks delete_repo");
        }
        if (aqsaSeeAbd.status !== 0) ok(`aqsa cannot see abdull private ${name}`);
        else fail("aqsa saw abdull private repo", aqsaSeeAbd.stdout);
      } else {
        const aqsaSee = run("gh", ["api", `repos/${AQSA.githubUser}/${name}`, "--jq", ".full_name"], {
          env: { ...env, GH_TOKEN: aqsaTok, GITHUB_TOKEN: undefined },
        });
        if (aqsaSee.status === 0) {
          note("warn", `Leftover throwaway still exists: ${AQSA.githubUser}/${name}`, "delete manually — token lacks delete_repo");
        }
        const abdSeeAqsa = run("gh", ["api", `repos/${AQSA.githubUser}/${name}`, "--jq", ".full_name"], {
          env: { ...env, GH_TOKEN: abdTok, GITHUB_TOKEN: undefined },
        });
        if (abdSeeAqsa.status !== 0) ok(`abdull cannot see aqsa private ${name}`);
        else fail("abdull saw aqsa private", abdSeeAqsa.stdout);
      }
    }
  }
  ...
  {
    acct(["profile", "ssh-key", ABDULL.id, "--path", ABDULL.sshKey], env);
    const inc = fs.readFileSync(path.join(configDir, "git", "personal.inc"), "utf8");
    if (/IdentitiesOnly=yes/.test(inc) && /abd_github/.test(inc)) ok("sshCommand pins abdull key");
    else fail("sshCommand", inc);
    // Without IdentitiesOnly, agent might offer wrong keys — we only verify config
    if (/IdentityFile=.*aqsa/.test(inc)) fail("personal.inc mentions aqsa key", inc);
    else ok("personal.inc does not reference aqsa key");
  }
