...
describe("readLocalAcct (I3 empty fail-closed)", () => {
  ...
  it("valid profile id", () => {
    fs.writeFileSync(path.join(tmp, ".acct"), "profile: aqsa\n");
    expect(readLocalAcct(tmp)).toEqual({ profile: "aqsa" });
  });
...
describe("findLocalAcct (nearest walk-up)", () => {
  ...
  it("finds .acct in cwd without a git repo", () => {
    ...
    expect(findLocalAcct(dir)).toEqual({ profile: "aqsa" });
  });
  ...
  it("walks up to nearest ancestor .acct", () => {
    ...
    fs.writeFileSync(path.join(parent, ".acct"), "profile: aqsa\n");
    expect(findLocalAcct(child)).toEqual({ profile: "aqsa" });
  });
  ...
  it("nearest child .acct wins over parent", () => {
    ...
    fs.writeFileSync(path.join(pkg, ".acct"), "profile: aqsa\n");
    expect(findLocalAcct(pkg)).toEqual({ profile: "aqsa" });
