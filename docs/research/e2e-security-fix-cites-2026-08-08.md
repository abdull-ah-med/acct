...
| Absolute helper path or `!` shell snippet is valid; shell quoting required for spaces | https://git-scm.com/docs/gitcredentials |
| `includeIf gitdir` matches location of `.git` (or `$GIT_DIR`) | https://git-scm.com/docs/git-config |
| Hooks live under `core.hooksPath`; `--no-verify` bypasses client hooks | https://git-scm.com/docs/githooks |
...
| `core.sshCommand` controls Git’s SSH transport independently of HTTPS helpers | https://git-scm.com/docs/git-config#Documentation/git-config.txt-coresshCommand |
...
Re-fetched 2026-08-08 while implementing ssh-test dual-plane, exec cross-profile guard, doctor effective-helper classification, and ACCT_DEBUG redaction.
